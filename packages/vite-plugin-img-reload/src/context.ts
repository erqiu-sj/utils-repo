 ...
 */
import {pathExists, ensureDir, readdir, createFile, writeFile} from 'fs-extra'
...
export class ImgReloadContext {
    ...
    processingResourcePathDir() {
        this.ops.resourcePathDir && readdir(this.ops.resourcePathDir, {encoding: 'utf-8'}).then(async res => {
            // @ts-ignore
